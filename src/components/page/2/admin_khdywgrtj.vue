                ...
                this.postRequest("/admin/listGatewayCustomerDayReport", {
                    ...
                    for (let item of res.data.data.data) {
                        self.tableData.push({
                            ...
                            submitSuccess: `${item.submitSuccess}条/${item.submitSuccMoney != 0 ? item.submitSuccMoney / 10000 : 0}元`,
                            submitFailed: `${item.submitFailed}条/${item.submitFailedMoney != 0 ? item.submitFailedMoney / 10000 : 0}元`,
                            submitSuccessRate: item.submitSuccessRate
                                ? (item.submitSuccessRate * 100).toFixed(4) + "%"
                                : 0,
                            reportSum: item.reportSum,
                            reportSuccess: `${item.reportSuccess}条/${item.reportSuccMoney != 0 ? item.reportSuccMoney / 10000 : 0}元`,
                            reportFailed: `${item.reportFailed}条/${item.reportFailedMoney != 0 ? item.reportFailedMoney / 10000 : 0}元`,
                            reportUnknow: `${item.reportUnknow}条/${item.reportUnknowMoney != 0 ? item.reportUnknowMoney / 10000 : 0}元`,
