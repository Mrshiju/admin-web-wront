...
<script>
    export default {
        ...
        methods: {
            ...
            exportData() {
                let self = this;
                this.postRequest("/admin/exportCustomerFeeMonthReport", {
            ...
            checkData() {
                ...
                this.loading = true;
                let self = this;
                this.postRequest("/admin/listCustomerFeeMonthReport", {
                    userName: this.userName,
                    agentName: this.agentName,
                    month: this.formatDate(this.month),
                    isFirst: this.isFirst,
                    ...
                    self.sumPage = Math.ceil(res.data.data.page.totalCount / self.pageSize);
                    for (let item of res.data.data.data) {
                        self.tableData.push({
                            ...
                            customerSubmitSum: `${item.customerSubmitSum}条/${item.customerSubmitSumMoney != 0 ? item.customerSubmitSumMoney / 10000 : 0}元`,
                            gatewaySubmitSum: item.gatewaySubmitSum,
                            gatewaySubmitSuccess: `${item.gatewaySubmitSuccess}条/${item.gateSubSuccMoney != 0 ? item.gateSubSuccMoney / 10000 : 0}元`,
                            gatewaySubmitFailed: `${item.gatewaySubmitFailed}条/${item.gateSubFailMoney != 0 ? item.gateSubFailMoney / 10000 : 0}元`,
                            gatewaySubmitSuccessRate:
                                (item.gatewaySubmitSuccessRate * 100).toFixed(4) + "%",
                            gatewayReportSum: item.gatewayReportSum,
                            gatewayReportSuccess: `${item.gatewayReportSuccess}条/${item.gateReportSuccMoney != 0 ? item.gateReportSuccMoney / 10000 : 0}元`,
                            gatewayReportFailed: `${item.gatewayReportFailed}条/${item.gateReportFailMoney != 0 ? item.gateReportFailMoney / 10000 : 0}元`,
                            platSubmitFailed: item.customerSubmitSum - item.gatewaySubmitSum,
                            gatewayReportUnknow: `${item.gatewayReportUnknow}条/${item.gateReportUnknowMoney != 0 ? item.gateReportUnknowMoney / 10000 : 0}元`,
                            gatewayReportSuccessRate:
                                (item.gatewayReportSuccessRate * 100).toFixed(4) + "%",
                            gatewayReportReturnRate:
